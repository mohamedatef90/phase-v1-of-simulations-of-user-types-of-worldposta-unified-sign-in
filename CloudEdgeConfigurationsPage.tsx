      ...
      <div className="flex justify-between items-start">
        ...
        <div className="flex space-x-2">
            <Button size="icon" variant="ghost" onClick={() => onEdit(config.id)} title="Edit">
                 <Icon name="fas fa-pencil-alt" ariaLabel="Edit configuration" />
            </Button>
            <Button size="icon" variant="ghost" onClick={() => onRemove(config.id)} title="Remove">
                <Icon name="fas fa-trash-alt" className="text-red-500 dark:text-red-400" ariaLabel="Remove configuration"/>
            </Button>
        </div>
      </div>
...
const CloudEdgeConfigurationsPage: React.FC = () => {
  ...
  const openAddModal = () => {
    setEditingConfig(null); // Ensure we are adding new, not editing
    setIsModalOpen(true);
  }
...
export default CloudEdgeConfigurationsPage;